...
describe("Branch", async function () {
    ...
    describe("Switch", async function () {
        ...
        let currenctBusinessId: string;
        let switchBusinessId: string;
        ...
        describe("Authenticate", async function () {
            ...
            it("authenticate", async function () {
                ...
                currenctBusinessId = result.data.businessId;
            });
        ...
        describe("Get business", async function () {
            ...
            it("row is an array", async function () {
                ...
                const filteredItem = result.data.rows.find(branch => branch.id !== currenctBusinessId);
                switchBusinessId = filteredItem ? filteredItem.id : "";
            });
        ...
        describe("Switch business", async function () {

            let result: ResponseSwitch;

            it("switch", async function () {
                const res = await request.post("/v1/business/switch")
                    .set("User-Agent", userAgent)
                    .set("Accept", "application/json")
                    .send({
                        id: switchBusinessId
        ...
        describe("Reauthenticate", async function () {
            ...
            it("switch successful", async function () {
                assert.strictEqual(currenctBusinessId, switchBusinessId, "Switched branch is not the same")
            });
        });
    });
});
