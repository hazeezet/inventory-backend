...
const root: FastifyPluginAsync = async (fastify): Promise<void> => {
	...
	fastify.get("/", async function (request, reply) {
		try
		{
			...
			const payload = request.PAYLOADS as ListTransferHistory;

			const branches = await lib.listTransferHistory(request.RAW_token, payload);
