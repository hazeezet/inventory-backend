...
import hooks from "#hooks/v1/item/adjustHistory/index.js";
import LIB_ITEM from "#services/v1/lib/item/index.js";
import DB_AUTH from "#services/v1/database/auth/index.js";
import { ListAdjustHistory } from "#types/payloads";

const root: FastifyPluginAsync = async (fastify): Promise<void> => {

	fastify.register(hooks);

	fastify.get("/adjust", async function (request, reply) {
		try
		{
			...
			const payload = request.PAYLOADS as ListAdjustHistory;

			const branches = await lib.listAdjustHistory(request.RAW_token, payload);
