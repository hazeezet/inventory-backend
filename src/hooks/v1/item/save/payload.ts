	...
	fastify.addHook<requestBody>("preHandler", async (request, reply) => {
		...
		try {
			...
			const req_payloads: AddItem = {
                ...
                Available: req_available ? parseFloat(req_available.toString().trim()) : undefined as unknown as number,
                Price: req_price ? parseFloat(req_price.toString().trim()) : undefined as unknown as number,
                Minimum: req_minimum ? parseFloat(req_minimum.toString().trim()) : undefined as unknown as number,
			...
			const payloads = await validate.addItem(req_payloads);
