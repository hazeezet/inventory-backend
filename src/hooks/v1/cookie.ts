...
const HOOK_COOKIE: FastifyPluginAsync = fp(async function (fastify, opts) {
	...
	fastify.addHook("preHandler", async (request, reply) => {
		try {
			...
			await DB.validateUseragent(payloads, request.headers["user-agent"] as string);
		}
		catch (error) {
