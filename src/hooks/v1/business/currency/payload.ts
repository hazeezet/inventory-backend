...
const HOOK_PAYLOAD: FastifyPluginAsync = fp(async function (fastify, opts) {
	...
	fastify.addHook<requestBody>("preHandler", async (request, reply) => {
		...
		try {
			const req_currency = request.body ? request.body.name : undefined as unknown as string;

			const validate = new validation(reply);

			const req_payloads: ChangeCurrency = {
				Name: req_currency ? req_currency.trim() : undefined as unknown as string
			}
			...
			const payloads = await validate.changeCurrency(req_payloads);
